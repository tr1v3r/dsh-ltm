...
| R7 CLI 管理 | `list/search/show/edit/tag/pin/merge/confirm/export/import` | 每个子命令 JSON（`--json`）与人类双输出；`--db` 指定库路径；未知/互斥 flag 与多余位置参数 fail-loud；export/import 保持完整记录状态并验证格式与 ID 冲突（集成测试）；export `--out` 只创建新文件，数据库与 SQLite 旁路路径保留（回归测试） |
| R8 一次性迁移 | 旧库 → 新 schema（仓库内脚本） | 一次性脚本 `scripts/legacy-migration/`（不随 npm 包发布，`migrate` 子命令已退役）：严格校验旧库 v1 表结构/`user_version=1`（源与快照双重校验），拒绝新版/同目标库；`MigrationReport.migratedCount == sourceCount`，原库文件字节不变（sha256 对比），pinned 记录迁移后可检索 |

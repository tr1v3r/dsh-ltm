...
但有 5 处 High：其中 3 处直接破坏 PLAN 中声明的核心承诺——R3 混合重排（H-1 实际退化为纯余弦）、
...
- **复现**（captain 探针，已确认）：
  ...
  - `"{{{{"` → `"{​{{​{"` — **仍含 `{{`**
  - `"{{"` → `"{​{"` — 正确（2 个时才对）
  用户粘贴的含 3+ 连续 `{` 的记忆文本会带着活的 `{{ … }}` 模板序列进入 recall 系统提示——正是 R9/PLAN §6 要拦的注入。
...
- **I-6 无秘密写日志**：surface 各文件 grep 无 `console`/logger 写记忆内容；`presentCall` 的 `rawInput` 是用户自身动作在 UI 卡展示，非日志。PLAN §6「不写日志」成立。类型层面 `count`/`.changes`/`RETURNING` 返回 JS number 非 BigInt。

---

## 建议修复顺序

1. **H-1**（重排丢 BM25，注意负号陷阱）、**H-3**（转义簇状绕过，安全）、**H-4**（预算倒置丢 pinned，含 M-1）——三处破坏 PLAN 核心承诺，优先。

...
- 本机为堵「记忆 → chezmoi 模板注入」养了两份本地 pnpm patch（`{{` 零宽空格转义），
  上游无法提交，只能长期自养——干脆自己写一个内置该能力的新插件。
...
| R9 | 提示词安全 | pinned 优先 + 字符预算机制保留；`{{` 零宽空格转义**内置**为 `escapeSequences` 配置项（默认 `['{{']`） |
...
├── tools.ts         # 模型侧工具（见 §4）
├── prompt.ts        # recall 分节渲染（pinned 优先 + 预算 + 转义）
...
| P1'（并行） | 插件面：config/tools/prompt/cli + `cordis.patch.yml` + README（先以 contracts+桩并行，最终联调） | P0 | 三旧工具兼容；转义有测试（含嵌套 `{{`、文本自带零宽空格边界）；config 非法值 load 即抛；CLI migrate 对 fixture 跑通 |
...
| P4 | 集成：仓库推 GitHub + CI（tag 门控发布，参照 dsh-quote-followup）；dotfiles 两 profile bundles 替换 `dsh-memory` → `dsh-ltm`，删两份本地 patch 及 patch 键；`dsh plugin install`；停旧库写入后 CLI migrate（备份先行）；真实 boot 两 profile（TUI 伪 TTY 横幅 + web 监听） | P3 | dump 无 `not found` 警告且新 entry 在树中；pinned 记忆迁移后可检索；**push/publish 前需本人确认** |
